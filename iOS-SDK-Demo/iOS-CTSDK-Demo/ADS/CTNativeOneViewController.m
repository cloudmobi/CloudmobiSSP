...
@interface CTNativeOneViewController () <CTNativeAdDelegate>
@property (nonatomic, strong)CTView *ccview;
@property (nonatomic, strong)ZCJHUD *hud;
@end

@implementation CTNativeOneViewController
@synthesize ccview;
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"NativeAD获取";
    ...
    [button addTarget:self action:@selector(getCTOne_ElementViewController) forControlEvents:UIControlEventTouchUpInside];
...
- (void)getCTOne_ElementViewController
{
    ...
    [service getNativeADswithSlotId:OneNativeSlotId delegate:self imageWidthHightRate:CTImageWHRateOnePointNineToOne isTest:YES success:^(CTNativeAdModel *nativeModel) {
        @strongify(self)
